	...
	addPanelActions: function(driver, device) {
		...
		if (panels[id] != null) {
			...
			panels[id].pm.events.on('zoneTripAlarm', function(data) {
				...
				let zone = data.zone.toString(); // Homey bug #1248
				Homey.manager('flow').triggerDevice('zoneTripAlarm', { zone: zone, name: name }, { state: data.state }, device, function(err, result) {
			...
			panels[id].pm.events.on('zoneBattery', function(data) {
				let name = self.getZoneName(device.id, data.zone);
				let zone = data.zone.toString(); // Homey bug #1248
				Homey.manager('flow').triggerDevice('zoneBattery', { zone: zone, name: name }, { state: data.state }, device);
			});
		}
			...
			if (panels[panel].pm.readAllSettings) {
				for (var idx in list) {
					...
					if ((type == 'magnet' || type == 'wired') && elem.stype == 'Magnet') {
						var capabilities = ['onoff', 'alarm_contact', 'alarm_tamper', 'alarm_generic'];
						if (type == 'magnet') {
							capabilities.splice(2, 0, 'alarm_battery');
						}
						items.push({
						...
						items.push({ 
							...
							capabilities: ['onoff', 'alarm_motion', 'alarm_battery', 'alarm_tamper', 'alarm_generic'],
							settings: setting
						});
