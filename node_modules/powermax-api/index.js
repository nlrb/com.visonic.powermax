...
var self = module.exports = {
		...
		if (panels[id] == null) {
			...
			powermax.events.on('download', function(state) {
				...
				if (state == 'process:MSG_DL_SERIAL') {
					...
					var newId = powermax.settings.config.panelSerial;
	...
	getValue: function(trigger, panel, zone, callback) {
		if (panels[panel] != null) {
			var zone = 'zone.' + zone;
			var val;
			if (panels[panel].panel[zone] != null) { // fix #3
				if (trigger == 'alarm_contact' || trigger == 'alarm_motion' || trigger == 'alarm_smoke') {
					val = panels[panel].panel[zone].trip;
					debug('Tripped: ' + val);
				} else if (trigger == 'alarm_battery') {
					val = panels[panel].panel[zone].battery;
					debug('Battery: ' + val);
				} else if (trigger == 'alarm_tamper') {
					val = panels[panel].panel[zone].tamper;
					debug('Tamper: ' + val);
				}
			}
			callback(null, val);
