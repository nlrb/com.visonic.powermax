...
var self = module.exports = {
	...
	addPanelActions: function(driver, device) {
		...
		if (panels[id] != null) {
			...
			panels[id].pm.events.on('zoneTripAlarm', function(data) {
				let name = self.getZoneName(device.id, data.zone);
				Homey.manager('flow').triggerDevice('zoneTripAlarm', { zone: data.zone, name: name }, { state: data.state }, device, function(err, result) {
					debug('Result zoneTripAlarm triggerDevice', err, result);
				});
			});
			// Catch battery events
			panels[id].pm.events.on('zoneBattery', function(data) {
				let name = self.getZoneName(device.id, data.zone);
				Homey.manager('flow').triggerDevice('zoneBattery', { zone: data.zone, name: name }, { state: data.state }, device);
			});
			panels[id].pm.events.on('eventLog', function() {
				Homey.manager('api').realtime('eventlog', { panel: id, log: panels[id].pm.eventLog });
			});
			panels[id].pm.events.on('busy', function(state) {
				Homey.manager('api').realtime('busy', { panel: id, busy: state });
			});
		}
		...
		if (panels[panel] != null) {
			...
			if (armCode != null) {
				...
				if (allowed && panels[panel].pm.readAllSettings) {
					...
					var instant = panels[panel].settings.armInstant || false;
	...
	addSensorDevice: function(driver, device, name) {
		...
		if (panels[panel] != null) {
			...
			panels[panel].pm.events.on('zone.' + did[1], function(field, newVal) {
				debug('Zone event', field, 'with value', newVal);
				...
				if (field == 'battery') {
					...
					let event = sensorAlarmMap[did[0]];
					driver.realtime(device, event, newVal, function(err, success) {
						debug('Real-time tripped update event', event, 'zone', did[1] + ':', (err ? err : 'OK'));
					});
				} else if (field == 'tripAlarm') {
					driver.realtime(device, 'alarm_generic', newVal, function(err, success) {
						debug('Real-time trip alarm update zone', did[1] + ':', (err ? err : 'OK'));
					});
				} else if (field == 'bypass') {
					driver.realtime(device, 'onoff', !newVal, function(err, success) {
						debug('Real-time bypass update zone', did[1] + ':', (err ? err : 'OK'));
					});
				}
			});
		} else {
	...
	getZoneValue: function(trigger, panel, zone, callback) {
		if (panels[panel] != null) {
			...
			if (panels[panel].pm.zone[zone] != null) { // fix #3
				if (trigger == 'alarm_contact' || trigger == 'alarm_motion'
					|| trigger == 'alarm_smoke' || trigger == 'alarm_co') {
					...
					debug('Tamper:', val);
				} else if (trigger == 'alarm_generic') {
					val = panels[panel].pm.zone[zone].tripAlarm;
					debug('Generic:', val);
				} else if (trigger == 'onoff') {
					val = !panels[panel].pm.zone[zone].bypass; // invert bypass
					debug('Bypass:', val);
				}
			}
			callback(null, val);
	...
	setZoneBypass: function(panel, zonenr, state) {
		if (panels[panel] != null) {
			if (panels[panel].settings.allowBypass) {
				let user = panels[panel].settings.armUser || 1;
				panels[panel].pm.setBypass(user, [ zonenr ], state);
				return true;
			} else {
				debug('Not allowed to update sensor bypass');
				return false;
			}
		}
	},
	...
	getEventLog: function(panel, force) {
		let result = false;
		if (panels[panel] != null) {
			result = panels[panel].pm.getEventLog(force);
		}
		return result;
	},
