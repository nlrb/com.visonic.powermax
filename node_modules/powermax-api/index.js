	...
	addPanel: function(driver, device, name, settings) {
		...
		powermax.events.on('download', function(state) {
			...
			if (panelSearch != null) {
				...
				if (state == 'done' && panels[id].children != null) {
		...
		if (panels[id] != null) {
			...
			panels[id].pm.events.on('system', function(field, newVal) {
				if (field == 'status') {
					...
					driver.realtime(device, 'arm_state', detail, function(err, success) {
					...
					for (var i in panels[id].pm.zone) {
						if (panels[id].pm.zone[i].alarm) {
							var name = self.getZoneName(id, i);
					...
					if (field == 'ready') {
						// Invert ready state to nready for mobile card
						field = 'nready';
						newVal = !newVal;
					}
