...
class Handlers extends EventEmitter {
	...
	updateStatus(what, field, newVal) {
		let idx = what + ':' + field;
		let curVal = this.zone.get(idx);
		let changed = curVal != null && typeof newVal == 'object' ? newVal.nr != curVal.nr : newVal != curVal;
		if (changed) {
			this.zone.set(idx, newVal);
			this.emit(what, field, newVal);
	...
	getStatus(what, field) {
		let result
		if (what !== undefined) {
		 	if (field !== undefined) {
				let idx = what + ':' + field
				result = this.zone.get(idx)
			} else {
				result = {}
				this.zone.forEach((value, key) => {
					let idx = key.split(':')
					if (idx[0] === what) {
						result[idx[1]] = value
					}
	...
	handleSettings() {
		if (this.lastSentPdu != null) {
			...
			this.emit('download', 'received:' + page + ',' + index);
			return true;
		}
		return false;
	}
	...
	handleEventlog() {
		...
		if (eventNum == 0x01) {
			...
			let event = {};
			if (this.powerMaster) {
				zoneStr = pm.logUserMaster[this.locale][eventZone] || "UNKNOWN";
				let utcSeconds = minNum + 0x100 * horNum + 0x10000 * dayNum + 0x1000000 * monNum;
				let d = new Date(0);
				d.setUTCSeconds(utcSeconds);
				partNum = secNum;
				secNum = d.getUTCSeconds();
				minNum = d.getUTCMinutes();
				horNum = d.getUTCHours();
				dayNum = d.getUTCDate();
				monNum = d.getUTCMonth() + 1;
				yerNum = d.getUTCFullYear();
			} else {
				zoneStr = pm.logUser[this.locale][eventZone] || "UNKNOWN";
			}
			let date = ('0' + dayNum).slice(-2) + '/' + ('0' + monNum).slice(-2) + '/' + yerNum;
			let time = ('0' + horNum).slice(-2) + ':' + ('0' + minNum).slice(-2);

			if (!this.powerMaster && pm.panelConfig.CFG_PARTITIONS[this.panelTypeNr] > 1) {
				for (let i = 1; i <= 3; i++) {
					partNum = (secNum % (2 * i) >= i) ? i : partNum;
				}
			} else {
			...
			let idx = eventNum - 1;
			event = { partition: (partNum == 0 ? "Panel" : partNum),
				date: date, time: time, zone: zoneStr, event: eventStr };
	...
	handleStatus() {
		...
		if (eventType == 0x01) { // Zone alarm status
			...
			for (let i = zoneLow; i <= zoneHigh; i++) {
			...
			for (let i = zoneLow; i <= zoneHigh; i++) {
				...
				if (this.settings.zones[i] != null) {
					if (this.updateStatus('zone.' + i, 'tamper', tamper)) {
						...
						this.emit('zoneTamper', { zone: i, state: tamper });
					}
				} else {
					if (tamper) {
	...
	handlePowerMaster() {
		...
		if (msgType !== undefined && subType !== undefined && msgLen !== undefined) {
			this.debug('Received PowerMaster message', msgType.toString(16) + '/' + subType.toString(16), '(len = ' + msgLen + ')');
		} else {
			this.debug('Received PowerMaster message', this.incomingPdu);
		}
		if (subType === 0x24 && msgLen > 9) {
			let temp = this.incomingPdu[8] + '.' + this.incomingPdu[9];
