...
var locale = Homey.manager('i18n').getLanguage() == 'nl' ? 'nl' : 'en'; // only Dutch & English supported
...
var self = module.exports = {
	...
	handleEventlog: function(state) {
		...
		if (eventNum == 0x01) {
			...
			var zoneStr = pm.logUser[locale][eventZone] || "UNKNOWN";
	...
	handleStatus: function(state) {
		...
		if (eventType == 0x01) { // Zone alarm status
			...
			for (var i = 1; i <= 30; i++) {
				var alarm = (zoneStatus & Math.pow(2 , (i - 1))) > 0;
				var tamper = (batteryStatus & Math.pow(2, (i - 1))) > 0;
				if (state.settings.zones[i] != null) {
					if (updateStatus(state, 'zone.' + i, 'alarm', alarm)) {
						state.debug('Updating tripped status for zone device ' + i + ' (' + alarm + ')');
					}
			...
			for (var i = 1; i <= 30; i++) {
				...
				if (state.settings.zones[i] != null) {
					...
					if (updateStatus(state, 'zone.' + i, 'battery', battery)) {
						...
						state.events.emit('battery', { zone: i, low: battery });
					};
				} else {
					if (trip) {
						state.debug('Sensor zone ' + i + ' tripped, but no device found!');
			...
			var statusText = pm.sysStatus[locale] == null ? pm.sysStatus.en[sysStatus] : pm.sysStatus[locale][sysStatus];
	...
	handlePanel: function(state) {
		...
		for (var i = 1; i<= msgCnt; i++) {
			...
			var s = (pm.logEvent[locale][eventType] || 'UNKNOWN') + ' / ' + (pm.logUser[locale][eventZone] || 'UNKNOWN');
			state.debug("System message: " + s);
			var eventIdx = '0x' + ('0' + eventType.toString(16)).slice(-2);
			var alarmStatus = pm.panelAlarmType[eventIdx];
			var troubleStatus = pm.panelTroubleType[eventIdx];
			updateStatus(state, 'system', 'event', { 
				nr: eventIdx, 
				type: pm.logEvent[locale][eventType], // type of event
				userType: eventZone,
				userText: pm.logUser[locale][eventZone],
				alarmType: alarmStatus,
				alarmText: pm.panelAlarmText[locale][alarmStatus],
				troubleType: troubleStatus,
				toubleText: pm.panelTroubleText[locale][troubleStatus]
			});
			updateStatus(state, 'system', 'alarmType', {
				nr: alarmStatus,
				loc: pm.panelAlarmText[locale][alarmStatus],
				txt: (alarmStatus !== undefined ? pm.logEvent[locale][eventType] : undefined)
			});
			updateStatus(state, 'system', 'troubleType', {
				nr: troubleStatus,
				loc: pm.panelTroubleText[locale][troubleStatus],
				txt: (troubleStatus !== undefined ? pm.logEvent[locale][eventType] : undefined)
			});
