...
function bytes2Dword(s, start) {
	return s[start] + 0x100 * s[start + 1] + 0x10000 * s[start + 2] + 0x1000000 * s[start + 3];
}

// Update a variable only if the value will change
function updateStatus(state, what, id, field, newVal) {
	if (id != null) {
		if (state.panel[what][id] == null) {
			state.panel[what][id] = [];
		}
		var curVal = state.panel[what][id][field];
		if (curVal != newVal) {
			state.panel[what][id][field] = newVal;
			state.events.emit(what, id, field, newVal);
			return true;
		}
   }
   return false;
...
var self = module.exports = {
	...
	handleAck: function(state) {
		if (state.lastSendMsg != null) {
			var lastSendMsgType = state.lastSendMsg[1];

			if (lastSendMsgType == 0x0F && state.readAllSettings == true) {
	...
	handleDenied: function(state) {
	   var lastSendMsgType = state.lastSendMsg[1];
	   state.debug("Handling DENIED message (" + lastSendMsgType + ")");
	   
	   if ((lastSendMsgType & 0xA0) == 0xA0) { // A0, A1, AA
		  //pmUserMessage("WRONG_PIN")
		  if (lastSendMsgType == 0xA0) {
			 //TODO remove:luup.variable_set(PANEL_SID, "EventLog", "*** Access denied ***", device)
		  }
	   } else if (lastSendMsgType == 0x24) {
	...
	handleDownloadRetry: function(state) {
	   ...
	   state.expectedResponse[0] = 0x25;
	   var pdu = [0x0D];
	   pdu.concat(pm.send.MSG_DOWNLOAD.msg, [0x00, 0x0A]);
	   state.msgDelayed = { pdu: pdu, resp: 0x3C };
	   //luup.call_delay("pmSendDelayed", waitTime, "") // start new thread
	   var scope = state;
	   setTimeout(function() { scope.sendDelayed(); }, waitTime);
	},
	...
	handleInfo: function(state) {
		...
		var model = pm.panelType[panelTypeNr] || "UNKNOWN";
		state.debug("Model: " + model + "; Nr: " + panelTypeNr + "; Master: " + (powerMaster ? "Yes" : "No"));
	...
	handleEventlog: function(state) {
		state.debug("Eventlog received");
		var eventNum = state.incomingPdu[3];
		if (eventNum == 0x01) {
			state.eventLog = { done: true, items: [], time: new Date() };
			...
			var zoneStr = pm.logUser[eventZone] || "UNKNOWN";
			//TODO:if (isPanelEvent == true) {
			//   pmEventLog = pmEventLog + "  * "
			//else
			//   pmEventLog = pmEventLog + "    "
			//end
			var eventStr = pm.logEvent[locale][logEvent] || "UNKNOWN";
			var idx = eventNum - 1;
			var event = {};
	...
	handleStatus: function(state) {
		...
		//}
		if (eventType == 0x02) { // Zone open status
			...
			for (var i = 1; i <= 30; i++) {
				var trip = (zoneStatus & (2 ^ (i - 1))) > 0 ? true : false;
				if (state.settings.zones[i] != null) {
					if (updateStatus(state,'zone', i, 'tripped', trip)) {
						state.debug("Updating tripped status for zone device " + i);
					}
					var battery = (batteryStatus & (2 ^ (i - 1))) > 0;
					updateStatus(state,'zone', i, 'battery', battery);
				} else {
					if (trip == 1) {
			...
			if (zoneEType != 0x00) {
				state.debug("Event " + (pm.eventType[locale][zoneEType] || "UNKNOWN") + " in Zone " + eventZone);
				...
				if (zone == null) {
					...
					if (zoneEType == 0x03) { // Open event door/window sensor
						updateStatus(state, 'zone', eventZone, 'tripped', true); // prevent updating twice; status msg comes before event msg
					} else if (zoneEType == 0x04) { // Close event door/window sensor
						updateStatus(state, 'zone', eventZone, 'tripped', false);
					} else if (zoneEType == 0x05) { // Motion event
						updateStatus(state, 'zone', eventZone, 'tripped', true);
			...
			for (var i = 0; i < 15; i++) {
				...
				var status = (x10Status & (2 ^ i)) > 0 ? true : false;
				if (updateStatus(state, 'x10', s, 'on', status) == true) {
			...
			updateStatus(state, 'system', state.id, 'ready', ready);
			updateStatus(state, 'system', state.id, 'memory', memory);
			updateStatus(state, 'system', state.id, 'trouble', trouble);
			updateStatus(state, 'system', state.id, 'alarm', alarm);
			if (!ready) { 
				sysFlagsStr += ",NRDY";
			}
			if (memory) { 
			...
			var armModeNum = (pm.isArmed[sysStatus] != null) ? 1 : 0;
			updateStatus(state, 'system', state.id, 'status', 
				{ armed: (armModeNum == 1), nr: sysStatus, txt: pm.sysStatus[locale][sysStatus] }
			);
			...
			for (var i = 0; i <= 7; i++) {
				if (sysFlags & (2 ^ i) != 0) {
					if (i == 5) {
						if (eventZone == 0xFF) {
							s += (pm.eventType[locale][zoneEType] || "UNKNOWN") + " from Panel, ";
					   } else {
							s += pm.eventType[locale][zoneEType] + " in Zone " + eventZone + ", ";
					   }
					} else {
						s += (pm.sysStatusFlags[locale][i] || "UNKNOWN") + ", ";
					}
				}
			}
			s = (s == "") ? "Not ready" : s.slice(-2);
			state.debug("System state is " + (pm.sysStatus[locale][sysStatus] || "UNKNOWN") + ' ' + sysFlagsStr);
			...
			for (var i = 1; i <= 30; i++) {
				var enrolled = (zoneEnrolled & (2 ^ (i - 1))) > 0;
				var bypass = (zoneBypass & (2 ^ (i - 1))) > 0;
				...
				if (sensor != null) {
					...
					api.debug("Found zone " + i + " to be enrolled while not registered.");
					pmSensorDev_t[i] = {};
	...
	handlePanel: function(state) {
		...
		for (var i = 1; i<= msgCnt; i++) {
			...
			var s = (pm.logEvent[locale][eventType] || 'UNKNOWN') + ' / ' + (pm.logUser[eventZone] || 'UNKNOWN');
			state.debug("System message: " + s);
			var eventIdx = '0x' + ('0' + eventType).slice(-2);
			var alarmStatus = pm.panelAlarmType[eventIdx] || 'None';
			var troubleStatus = pm.panelTroubleType[eventIdx] || 'None';
...
/*
	...
	handlePowerMaster: function(state, api) {
	   ...
	   var msgType = string.byte(pmIncomingPdu, 3) // 00, 01, 04: req; 03: reply, so expecting 0x03
	   var subType = string.byte(pmIncomingPdu, 4)
	   var msgLen = string.byte(pmIncomingPdu, 5)
	   api.debug(string.format("Received PowerMaster message %02X/%02X (len = %02X)", msgType, subType, msgLen))
	   return true
	}
*/
}