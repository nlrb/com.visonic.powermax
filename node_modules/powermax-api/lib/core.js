...
/*
   ...
   - zone: a zone sensor has changed state; param: id, field, newVal (id = zone nr, 
		field = ['tripped', 'lastTrip', 'battery', 'bypass'])
...
// Main constructor
function PowerMax(ip, port, debug) {

	this.id = ip + ':' + port;
	this.debug = function(text) { debug('[' + this.id + '] ' + text); };
	this.client;
	this.found = false;
	// Event handling
...
PowerMax.prototype.powerlinkEnrolled = function() {
	...
	if (this.powerMaster == true) {
		this.sendMessage("MSG_DL", { item: pm.download.MSG_DL_MR_SIRKEYZON });
	}
	var syncTime = true; //(luup.variable_get(PANEL_SID, "AutoSyncTime", pmPanelDev) != "0")
	if (syncTime == true) {
		var t = new Date();
