...
/*
   ...
   - system: the panel system state has changed; param: id, field, newVal (id = 0, 
		field = ['status', 'nready', 'memory', 'trouble', 'alarm'])
		...
		if (state.found && self.intervalTimer === undefined) {
			...
			self.intervalTimer = setInterval(function() {
				...
				if (self.powerlinkMode && !self.starting) {
					var delta = now - (self.lastKeepAlive || 0);
				...
				if (timeout && self.expectedResponse != '') {
					// resend last message
					self.debug('*** Re-sending PDU (got no response) ***');
...
PowerMax.prototype.sendMessage = function(msg, modifiers, immediate) {
	...
	if (msg == -1) { // re-send last message
		...
		for (var i = 0; i < msgItem.msg.length; i++) {
			if (typeof msgItem.msg[i] != 'string') {
				outPdu.push(msgItem.msg[i]);
			} else {
				// handle modifier
				outPdu = outPdu.concat(modifiers[msgItem.msg[i]]);
			}
		}
