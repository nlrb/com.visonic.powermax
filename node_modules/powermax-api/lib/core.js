...
class PowerMax extends Handlers {
	...
	*/
		this.events.on('download', (state) => {
			this.downloadState = state; // can be 'start', 'failed' or 'done'
		});
		this.events.on('found', (state) => {
	...
	openConnection(ip, port) {
		...
		this.client.on('timeout', () => {
			...
			this.events.emit('found', { found: this.found, err: 'Timed out' });
			this.closeConnection();
		});
		// Handle closed connections, try to re-open it
		this.client.on('close', () => {
			this.debug('Connecion closed (found =', this.found + ')');
			this.events.emit('found', { found: false, err: 'Connection closed' });
	...
	getZoneStatus(zone) {
		if (this.state !== undefined && this.state[zone] !== undefined) {
			this.events.emit('zone.' + zone, 'trip', this.state[zone].trip)
			this.events.emit('zone.' + zone, 'battery', this.state[zone].battery)
			this.events.emit('zone.' + zone, 'tamper', this.state[zone].tamper)
			this.events.emit('zone.' + zone, 'bypass', this.state[zone].bypass)
			this.events.emit('zone.' + zone, 'tripAlarm', this.state[zone].tripAlarm)
			this.events.emit('zone.' + zone, 'tamperAlarm', this.state[zone].tamperAlarm)
		}
