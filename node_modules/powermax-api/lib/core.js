...
// Main constructor
function PowerMax(id, settings, debug) {
	...
	this.debug = function() { debug('[' + self.ip_port + ']', arguments); };
	this.client;
...
PowerMax.prototype.getEventLog = function(force) {
	...
	if (!force && this.eventLog.done) {
		...
		if (this.settings && this.settings.config && this.settings.config.installerCode) {
			let rawPin = this.settings.config.installerCode.val;
			let pin = [ Number('0x'+ rawPin.slice(0,2)), Number('0x'+ rawPin.slice(-2)) ];
			this.debug(pin, this.settings.config.installerCode);
			if (pin !== undefined) {
