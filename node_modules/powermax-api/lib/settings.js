				...
				for (let i = 0; i < zoneCnt; i++) {
					...
					if (zoneEnrolled == true) {
						...
						if (this.state.powerMaster == false) {  // PowerMax models
							zoneInfo = item[i * 4 + 3];
                            sensorId = item[i * 4 + 2];
                            let sensorIdString = '0x' + sensorId.toString(16)
                            if (pm.zoneSensorPowermax[sensorId] !== undefined) {
                                sensorType = pm.zoneSensorPowermax[sensorId].func
                                sensorName = pm.zoneSensorPowermax[sensorId].name
                            }
                            else {
						...
						this.debug('Zone ' + (i + 1) + ' (' + zoneName + '): ' + pm.zoneType[state.locale][zoneType] + ' (chime = '
							+ pm.zoneChime[state.locale][zoneChime] + ' ; sensor type = ' + sensorId.toString(16) + ' [' + sensorType + '])');
						this.zones[i + 1] = {
							stype: sensorType,
							sid: sensorId,
