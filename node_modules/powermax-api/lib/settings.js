...
settings.prototype.process = function() {
	var state = this.state;
	this.debug('Processing settings...');
...
*/	  
		...
		if (item.length != 0) {
			...
			for (var i = 0; i < userCnt; i++) {
				this.config.userCode[i] = item.slice(2 * i, 2 * i + 2);
			}
		}
		...
		if (item.length != 0) {
			...
			var panelName = pm.panelName[idx] || "UNKNOWN";
			var panelserial = '';
			for (var i = 0; i < 5; i++) {
			...
			this.debug('Panel ' + panelName+ ' with serial ' + panelserial);
			this.config.panelName = panelName;
		...
		if (item.length != 0 && zoneNames.length != 0) {
			for (var i = 0; i < zoneCnt; i++) {
				...
				if (zoneEnrolled == true) {
					var zoneInfo, sensorId, sensorType;
					if (this.state.powerMaster == false) {  // PowerMax models
						zoneInfo = item[i * 4 + 3];
						sensorId = item[i * 4 + 2];
						sensorType = pm.zoneSensor['0x' + (sensorId & 0xF).toString(16)] || "UNKNOWN";
					} else { // PowerMaster models
						zoneInfo = item[i];
						sensorId = itemMr[i * 10 + 6];
						sensorType = pm.zoneSensorMaster['0x' + (sensorId & 0xF).toString(16)].func || "UNKNOWN";
					}
					...
					/*
					if (sensorType == "Magnet") or (sensorType == "Wired") {
						doorZoneStr = string.format("%s,Z%02d", doorZoneStr, i)
					} else {if (sensorType == "Motion") or (sensorType == "Camera") {
						motionZoneStr = string.format("%s,Z%02d", motionZoneStr, i)
					} else {if (sensorType == "Smoke") or (sensorType == "Gas") {
...
/*
	  ...
	  var x10Names = pmReadSettings(pmDownloadItem_t.MSG_DL_X10NAMES)
	  if (setting != "") and (x10Names != "") {
		 for i = 0, 15; i++) {
			var enabled = false
			var x10Name = 0x1F
			for j = 0, 8; i++) {
			...
			if (enabled == true) or (x10Name != 0x1F) {
			   if (i == 0) {
				  deviceStr = deviceStr .. ",PGM"
			   } else {
				  deviceStr = string.format("%s,X%02d", deviceStr, i)
				  if (string.find(devices, string.format("X%02dd", i)) != null) {
